...
\texttt{clang-format} with the appropriate arguments and reformat the files in-place (if necessary) -- i.e. your source files may be modified. Don't run this tool if you don't want this to happen. You can specify particular source
files on the command line, or, if none are specified, then the tool tries to reformat all of the .c and .h files in the current
directory.
...
\subsection{Tabs vs spaces \protect\hfill\icon{CLANG-FORMAT}}
...
\subsection{Indent size \protect\hfill\icon{CLANG-FORMAT}}
...
\subsection{Comments \protect\hfill\icon{CLANG-FORMAT}}
...
\subsection{Continuation lines \protect\hfill\icon{CLANG-FORMAT}}
...
\subsection{Continuation lines for nested expressions \protect\hfill\icon{CLANG-FORMAT}}
...
\subsection{\texttt{for} loops \protect\hfill\icon{CLANG-FORMAT}} 
...
\subsection{\texttt{switch} statements \protect\hfill\icon{CLANG-FORMAT}}
...
\subsection{Continuation lines for braced initialiser lists \protect\hfill\icon{CLANG-FORMAT}}
...
\subsection{Continuation lines for string constants \protect\hfill\icon{CLANG-FORMAT}}
...
\subsection{Unary operators \protect\hfill\icon{CLANG-FORMAT}}
...
\subsection{Binary and ternary operators \protect\hfill\icon{CLANG-FORMAT}}
...
\subsection{Control statements and functions \protect\hfill\icon{CLANG-FORMAT}}
...
\subsection{Spaces at the ends of lines \protect\hfill\icon{CLANG-FORMAT}}
...
\subsection{Pointer declarations \protect\hfill\icon{CLANG-FORMAT}}
...
\subsection{Array derefencing \protect\hfill\icon{CLANG-FORMAT}}
...
\lstinputlisting[label=lst:space4]{examples/space5.c}
\linenumbers


\subsection{Spaces before comments \protect\hfill\icon{CLANG-FORMAT}}
...
\subsection{Line termination character \protect\hfill\icon{CLANG-FORMAT}}
...
\subsection{Use of blank lines \protect\hfill\icon{CLANG-FORMAT}\icon{MANUAL-CHECK}}
...
\subsection{One statement per line \protect\hfill\icon{CLANG-FORMAT}}
...
\subsection{Multiple assignment operators in the one statement \protect\hfill\icon{MANUAL-CHECK}}
...
\subsection{Use of the comma operator \protect\hfill\icon{MANUAL-CHECK}}
...
\subsection{Function definitions \protect\hfill\icon{CLANG-FORMAT}}
...
\subsection{Control statements \protect\hfill\icon{CLANG-FORMAT}}
...
\lstinputlisting[label=lst:brace1]{examples/brace2.c}
\linenumbers

\section{Naming Conventions}
\subsection{Variable names \protect\hfill\icon{CLANG-TIDY}}
...
\subsection{Defined constants and macros \protect\hfill\icon{CLANG-TIDY}}
...
\subsection{Function names \protect\hfill\icon{CLANG-TIDY}}
...
\vspace{-0.2cm}
\subsection{Type names \protect\hfill\icon{CLANG-TIDY}}
...
\subsection{Enumerated Types \protect\hfill\icon{CLANG-TIDY}}
...
\subsection{File names \protect\hfill\icon{MANUAL-CHECK}}
...
\section{Comments \protect\hfill\icon{MANUAL-CHECK}}
...
\subsection{Function Length \protect\hfill\icon{CLANG-TIDY}}
...
\subsection{Global variables \protect\hfill\icon{CLANG-TIDY}\icon{MANUAL-CHECK}}
\label{sec:global}
...
\subsection{Magic Numbers \protect\hfill\icon{CLANG-TIDY}}

``Magic numbers'' are to be avoided. Defined constants should be used instead of number and character literals scattered throughout code without context. 0 and 1 are almost never magic numbers.
...
\subsection{Duplicate Includes \protect\hfill\icon{CLANG-TIDY}}

Do not \texttt{\#include} any include file more than once in a source file.

% else after return
\subsection{\texttt{else} after \texttt{return}, \texttt{continue}, etc. \protect\hfill\icon{CLANG-TIDY}}
...
\lstinputlisting[label=lst:elseAfterReturn1]{examples/else-after-return2.c}
\end{multicols}
\linenumbers

% Inconsistent function parameter names
\subsection{Function parameter names \protect\hfill\icon{CLANG-TIDY}}

Parameter names must be present in function declarations and the names must match those used 
in the function definition. 

An unused function parameter may have its name commented out in the function 
definition but the name must be present in the declaration. (You should review the code to determine if the 
function parameter should be present in the first place.)
...
\subsection{Use of \texttt{const} for pointer function parameters \protect\hfill\icon{CLANG-TIDY}}
...
\subsection{Bug prone code \protect\hfill\icon{CLANG-TIDY}}
...
\subsection{Compilation \protect\hfill\icon{MANUAL-CHECK}}
...
\subsection{File encoding \protect\hfill\icon{MANUAL-CHECK}}
All source files must use an ASCII-compatible encoding. 

\subsection{Modularity \protect\hfill\icon{MANUAL-CHECK}}
...
\subsection{Banned Language Features  \protect\hfill\icon{CLANG-TIDY}\icon{MANUAL-CHECK}}
