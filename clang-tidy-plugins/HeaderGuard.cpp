...
public:
  HeaderGuardPPCallbacks(Preprocessor *PP, clang::tidy::csse2310::HeaderGuardCheck2310 *Check)
      : PP(PP), Check(Check) {}
  ...
  void EndOfMainFile() override {
    ...
    for (const auto &MacroEntry : Macros) {
      ...
      if (!FixIts.empty()) {
        if (!CurHeaderGuard.ends_with(NewGuard)) {
        // If the current header guard string does not end with the new guard
        // string then output a message
  ...
  std::string checkHeaderGuardDefinition(SourceLocation Ifndef,
    ...
    if (Ifndef.isValid() && !CurHeaderGuard.ends_with(CPPVar) &&
...
private:
  ...
  Preprocessor *PP;
  HeaderGuardCheck2310 *Check;
};
//} // namespace

void HeaderGuardCheck2310::registerPPCallbacks(const SourceManager &SM,
                                           Preprocessor *PP,
                                           Preprocessor *ModuleExpanderPP) {
  PP->addPPCallbacks(std::make_unique<HeaderGuardPPCallbacks>(PP, this));
}

std::string HeaderGuardCheck2310::sanitizeHeaderGuard(StringRef Guard) {
  // Only reserved identifiers are allowed to start with an '_'.
  return Guard.ltrim('_').str();
}

bool HeaderGuardCheck2310::shouldSuggestEndifComment(StringRef FileName) {
  return utils::isFileExtension(FileName, HeaderFileExtensions);
}

bool HeaderGuardCheck2310::shouldFixHeaderGuard(StringRef FileName) { return true; }

bool HeaderGuardCheck2310::shouldSuggestToAddHeaderGuard(StringRef FileName) {
  return utils::isFileExtension(FileName, HeaderFileExtensions);
}

std::string HeaderGuardCheck2310::formatEndIf(StringRef HeaderGuard) {
