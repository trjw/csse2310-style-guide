...
Every \texttt{.c} and \texttt{.h} source file must compile on its own. If a file does not compile (i.e. errors are produced) then other style aspects can not be checked by \texttt{2310stylecheck.sh}. 

\subsection{Compilation warnings\texorpdfstring{\protect\hfill\icon{STYLE-CHECK}}{}}
\label{sec:compilation-warnings}

Every \texttt{.c} and \texttt{.h} source file must compile on its own \ul{without warnings} when compiled with the \texttt{-Wall -Wextra -pedantic -std=gnu99} compiler arguments. 
...
An example file comment is shown below:

\nolinenumbers
\lstset{caption={Example file comment -- for a file named \texttt{program.c}}}
\lstinputlisting[label=lst:filecomment]{examples/filecomment.c}
\linenumbers

\subsection{Function comments\texorpdfstring{\protect\hfill\icon{DOXYGEN}}{}}
...
\item what the function returns (again, this is not a repetition of the type being returned, but a description of what the return value means);
...
\item any error conditions (the conditions under which a returned value indicates an error, or the circumstances in which the function does not return at all -- i.e. the program exits); and
\item how AI was used in the development of the function.
\end{itemize}
...
\item \texttt{@brief \textit{description}}
    \begin{itemize}
    \item The command gives a brief description of what the function does. This can be omitted if the description of the return value contains sufficient information. The \texttt{@brief} command itself is not required -- any text in the function comment prior to the first command or blank line is assumed to be the brief description.
    \end{itemize}
...
\item \texttt{@error \textit{description}}
    \begin{itemize}
    \item If there are circumstances (usually user errors) in which this function may exit (i.e. not return control to the caller but terminate the whole program) then a description must be provided of when and/or why this happens. This includes if functions called by this function may exit. If a function is intended to never return, then this can be specified with the \texttt{@return} command (see above). The \texttt{@error} command should be used when a function sometimes does not return. 
    \end{itemize}
    
\item \texttt{@ref \textit{description}}
    \begin{itemize}
    \item If you have copied code or taken inspiration from some source and the assignment requires a reference then this must be present in an \texttt{@ref} command. You must describe the source of the code or inspiration and which part of your function it relates to. 
    \end{itemize}
...
Some examples of function comments are shown below.

\nolinenumbers
\lstset{caption={Example function comment. The first line is the brief description of the function. The \texttt{@ai} command is present because it is assumed here that the file comment had an \texttt{@ai} command other than \texttt{@ai Not Used}.}}
\lstinputlisting[label=lst:functioncomment1]{examples/functioncomment1.c}
\linenumbers

\nolinenumbers
\lstset{caption={Example function comment.}}
