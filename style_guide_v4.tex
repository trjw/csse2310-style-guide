...
\noindent\icon{STYLE-CHECK} -- indicates that \texttt{2310stylecheck.sh} will check compliance with this style requirement. Any corrections must be applied manually. Note that \texttt{2310stylecheck.sh} will also run \texttt{clang-format}, \texttt{clang-tidy} and \texttt{doxygen}.
...
Your C programs must use the C99 standard for maximum code portability between POSIX systems. Some GNU extensions are allowed. Your programs must be compiled with the following flags as a minimum:
...
Every \texttt{.c} and \texttt{.h} source file must compile \ul{on its own}. If a file does not compile (i.e. errors are produced) then other style aspects cannot be checked by \texttt{2310stylecheck.sh}. 
...
\subsection{Line ending\texorpdfstring{\protect\hfill\icon{CLANG-FORMAT}}{}}
Source files must use Linux style line endings, i.e. lines must end with with a linefeed (newline) character only. Carriage return characters must not be present. 
...
\begin{itemize}
\itemsep -2pt
    \item At the start of source files
    \item Function declarations or definitions (see \hl{below} for which)
...
A sample file comment is shown in Example \ref{lst:filecomment}. Other doxygen formats are permissible. 
