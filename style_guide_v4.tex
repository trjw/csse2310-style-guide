...
\texttt{clang-tidy} and \texttt{doxygen} can be used to check many of these requirements. Scripts are available to do this (see below).
...
\noindent\icon{DOXYGEN} -- indicates that \texttt{doxygen} (with the appropriate arguments) can be use to check for compliance with this style (commenting) requirement. Any corrections must be applied manually (e.g. by adding an appropriate documentation comment).
...
All source files (both \texttt{.c} and \texttt{.h} files) are expected to individually compile without warnings when compiled with the \texttt{-Wall -Wextra -pedantic -std=gnu99} compiler arguments. 
...
\itemsep -2pt
    ...
    \item Function declarations or definitions (see below for which)
...
Required comments use doxygen format and are described below. Doxygen is an automatic documentation generator --  see \url{https://doxygen.nl/}. You are not required to generate documentation files from your code in CSSE2310/CSSE7231 but use of this format allows automated checking for required comments. 
...
Every \texttt{.c} and \texttt{.h} file must begin with a doxygen comment containing the \texttt{@file} command as the first command in the comment. This must list the name of the file (excluding any directory part). This is required for doxygen to be able to check other comments in the file.
...
\begin{itemize}
\itemsep -2pt
\item \texttt{@ai Not Used} -- indicates that AI was not used in any way in the development of this file. No other \texttt{@ai} commands must be present.
