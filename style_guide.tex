...
\title{CSSE2310/CSSE7231\\C Programming Style Guide\\Version 2.1.0}
...
Variable names and file names will begin with a lowercase letter and names with multiple words 
will use initial capitals for subsequent words and no underscores. 
Names that are chosen should be meaningful. Hungarian notation\footnote{Hungarian notation is where the name 
or a prefix or suffix within the name indicates the type of the variable. This can include single letters, e.g.
integer variable names beginning with an `i' or pointer variable names ending with `p'.}
 is NOT to be used for variable names.
...
Unacceptable variable names (use of Hungarian notation): \texttt{FILE *fp; char *aPtr; char *thingStr;}
...
where \texttt{alpha_sort} is the name of a function.

\subsection{Type names}
\label{sec:type-names}
Type names (i.e. from typedefs), structure and union names should begin with capital letters 
and use initial capitals for subsequent words and no underscores.
...
Function comments should not include parameter types (these are apparent from the function prototype/definition), 
nor should they repeat the function prototype. If an adequate comment is given for a function 
declaration, it need not be repeated for the associated function definition, even if these are in 
different files. If a declaration (prototype) and
definition are in the same file then either may be commented. If the declaration and definition are in different files
...
See examples~\ref{lst:braces-function} and~\ref{lst:vertical-whitespace} below for examples of function 
comments.
 ...
 * Return value will not be meaningful if the product can't be
 * represented in an integer, i.e overflows the bits available.
 */
int multiply(int x, int y) {
    return x * y;
}

// Divide x by y. It is assumed that y does not equal zero.
