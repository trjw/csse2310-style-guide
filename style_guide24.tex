...
\subsection{Continuation lines for nested expressions and initialisers}

Where an expression or initialiser is nested (e.g. has subcomponents grouped by parentheses or braces) then further continuation lines must be indented

\subsection{Continuation lines for boolean expressions}
Boolean expressions that span multiple lines, must (a) have their operators (i.e. \texttt{||} or \texttt{\&\&}) on the left side of the line, and (b) align
