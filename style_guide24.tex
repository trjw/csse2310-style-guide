...
Programs written for CSSE2310/7231 assignments must follow the style guidelines outlined in this document. The formatting requirements are based on the \texttt{WebKit} style as implemented by \texttt{clang-format}. There are also
...
that must be created for CSSE2310/7231, e.g. \texttt{Makefile}s. 
...
beyond the first line of the statement. (In other words, if a statement spills on to a third line, the indentation 
level of the third line will be the same as the second line -- i.e. four spaces beyond that of the first line of 
the statement.)

\subsection{Continuation lines for boolean expressions}
Boolean expressions that span multiple lines, must (a) have their operators on the left side of the line, and (b) align
operators at the same level in the expression, and (c) further indent nested expressions if required (i.e. those at a lower
level in the expression). 

\subsection{Continuation lines for function calls and braced initialiser lists}
If an initialisation list or the arguments to a function call don't fit on one line, but would completely fit on the next line (indented by four spaces) then they shall all be placed on the next line. If they will not all fit on the next line then they
must start on the original line and then continue to the next line.
...
label must be on a newline and indented by four spaces beyond the start of the \texttt{case} label.

\subsection{Indentation examples}
\lstset{numbers=left, numberstyle=\ttfamily, caption={Correct indentation for if statement and loop}, label={lst:indentation-examples}}

\nolinenumbers
\begin{lstlisting}[mathescape=true]
if (day == 31) { // the following statements are indented by four spaces
    monthTotal = 0;
    for (week = 0; week < 4; ++week) { // the following statement is indented by four spaces
